...
function MockTestEditor({ userData, role }) {
  ...
  const descriptions = [
    "you must select a course for which you want to create mock test",
    "",
    "",
    "Add,Update or Delete questions for the module you have selected",
  ];
  ...
  async function getMockResults(a) {
    ...
    const { data, error } = await supabase.rpc("get_mock_results", {
      p_test_id: a,
    });
        ...
        const isCorrect =
          question.type === "options"
            ...
            : question.options.answer?.trim().toLowerCase() ===
              reportItem.value?.trim().toLowerCase();
        if (isCorrect) {
    ...
    await Promise.all(
      (data || []).map(async (i) => {
        const { data: fullResult, error: fullResultError } = await supabase
          .from("mock_plays")
          .select("report")
          .eq("uid", i.uid)
          .single();
        if (fullResult && fullResult.report) {
          scoreMap[i.uid] = calculateScore(fullResult.report);
        } else {
          scoreMap[i.uid] = 0;
        }
      }),
  ...
  async function getSubmissionCount(a) {
    setSubCount(true);
    const { data, error } = await supabase.rpc("count_mock_submissions", {
      test_id_value: a,
    });
    if (data) {
      setSubCount(data);
  ...
  async function getSelectedModules(a) {
    ...
    const { data, error } = await supabase
      .from("mock_groups")
      .select("*")
      .match({ type: "module", parent_sub: a })
  ...
  async function getQuestions(a) {
    const { data, error } = await supabase
      .from("mock_questions")
      .select("*")
      .match({ isActive: true, parent: a || modules[activeModule]?.id })
      .order("seq", { ascending: true });
    if (data) {
  ...
  return (
    <div className="text-left sf w-full flex h-full flex-col overflow-auto">
      ...
      <h2 className="w-full text-left font-bold text-2xl ">
        {headings[views] || ""}
      </h2>

      <h4 className="w-full text-left text-gray-500 font-normal text-sm ">
        {descriptions[views] || ""}
      </h4>
      {/* Course Selector */}
      ...
      {views == 0 && tests != undefined ? (
        <div className="pr-2 mt-4 overflow-y-auto">
          ...
          <Popover>
            <PopoverTrigger>
              <Button className="mr-auto" size="sm" color="primary">
                Add a Category
              </Button>
            </PopoverTrigger>
            <PopoverContent className="p-2">
              <Input
                ...
                label="Title"
                onChange={(e) => {
                  setCategoryData((res) => ({ ...res, title: e.target.value }));
                }}
              ></Input>
              <Button
                size="sm"
                ...
                onPress={() => {
                  addCategory(categoryData?.title);
                }}
              >
                Add Category
              </Button>
              ...
              return (
                <>
                  ...
                  tests?.filter((item) => item.category == z.id) == 0 ? (
                    <div className="rounded-xl border-gray-300 border-1 text-gray-600 my-2 p-2 text-center bg-gray-50">
                  ...
                  {tests &&
                    tests
                      ?.filter((item) => item.category == z.id)
                      ?.map((i, d) => {
                        return (
                          <div className="w-full rounded-md border-1 border-gray-100 flex flex-row justify-between py-2 px-2 shadow-sm items-center my-1">
                            ...
                            <div className="flex flex-row">
                              ...
                              >
                                ...
                                <PopoverContent>{subCount}</PopoverContent>
                              </Popover>
                              <Button
                                size="sm"
                                className="ml-2"
                              ...
                              <Popover
                                ...
                                onOpenChange={(e) => {
                                  e == true ? setEditTest(i) : setEditTest();
                                }}
                              >
                                <PopoverTrigger>
                                  <Button
                                ...
                                <PopoverContent className="sf min-w-[300px] max-w-[500px] max-h-[50vh] overflow-y-scroll p-3 flex flex-col justify-start items-start">
                                  ...
                                  <DatePicker
                                    ...
                                    value={
                                      editTest?.start_time
                                        ? getParsedDate(editTest?.start_time)
                                        : null
                                    }
                                    onChange={(e) => {
                                      setEditTest((res) => ({
                                  ...
                                  <DatePicker
                                    ...
                                    value={
                                      editTest?.end_time
                                        ? getParsedDate(editTest?.end_time)
                                        : null
                                    }
                                    onChange={(e) => {
                                      setEditTest((res) => ({
                                      ...
                                      if (i.type == "slider") {
                                        return (
                                          <Slider
                                            getValue={(value) => {
                                              return (
                                                <p>{value / 60} minutes</p>
                                              );
                                            }}
                                            maxValue={18000}
                                            value={editTest?.config[i?.key]}
                                            onChange={(e) => {
                                              setEditTest((prevState) =>
                                                _.set(
                                                  { ...prevState },
                                  ...
                                  <Button
                                    className="mt-2 text-white flex-shrink-0"
                                    color="primary"
                                    size="sm"
                                    onPress={(e) => {
                                      updateTest(editTest, i?.id);
                                    }}
                                  >
                                    Update Test
                                  </Button>
                              ...
                              <Popover>
                                <PopoverTrigger>
                                  <Button
                                    size="sm"
                                    className="ml-2 p-1.5 text-white"
                                    color="danger"
                                    isIconOnly
                                  >
                                    <svg
                  ...
                  <Popover>
                    <PopoverTrigger>
                      <Button
                        className="w-auto mr-auto my-4 text-black"
                        color="secondary"
                        size="sm"
                        onPress={() => {
                          courses
                          ...
                          controls.map((i, d) => {
                            ...
                            if (i.type == "slider") {
                              return (
                                <Slider
                                  getValue={(value) => {
                                    return <p>{value / 60} minutes</p>;
                                  }}
                                  maxValue={18000}
                                  value={testData && testData[i?.key]}
                                  onChange={(e) => {
                                    setTestData((res) => ({
                                      ...res,
                                      [i.key]: e,
      ...
      {views == 1 && activeCourse != undefined && subjects != undefined ? (
        <div>
          <div className="bg-gray-200 rounded-md p-3 mb-4">
            ...
            {selectedsubjects &&
              selectedsubjects.map((i, d) => {
                return (
                  <div className="flex flex-row w-full justify-between bg-white border-1 rounded-md p-2 items-center align-middle">
                    <div className="flex flex-row items-center justify-start">
                      <div className="flex flex-row items-center justify-start">
                        <Input
                          type="number"
                          ...
                          endContent={
                            <Button
                              className="p-2"
                              onPress={() => {
                                updateSectionOrder(i?.seq, i.id, i.test);
                              }}
                              size="sm"
                              color="primary"
                              isLoading={bgLoading}
                            ...
                            >
                              ...
                              >
                                <desc>
                                  Check Streamline Icon:
                                  https://streamlinehq.com
                                </desc>
                                <g id="check--check-form-validation-checkmark-success-add-addition-tick">
                                  <path
                                    id="Vector 2356 (Stroke)"
                                    fill="#fff"
                                    fill-rule="evenodd"
                                    d="M23.76 5.7883L8.5452 21.0031L0.24 12.6979L3.0315 9.9064L8.5452 15.4202L20.9686 2.9969L23.76 5.7883Z"
                                    clip-rule="evenodd"
                                    stroke-width="1"
                                  ></path>
                                </g>
                        ...
                        <Input
                          ...
                          endContent={
                            <Button
                              className="p-2"
                              onPress={() => {
                                updateSectionTime(i?.time, i.id, i.test);
                              }}
                              size="sm"
                              color="primary"
                              isLoading={bgLoading}
                      ...
                      <p className="bg-primary text-white text-xs px-4 py-1 rounded-full mx-2">
                        {extractLabel(i?.subject) ?? "No Label"}
                      </p>
                    </div>
                    <div className="flex flex-row items-center justify-start">
            ...
            {selectedsubjects && selectedsubjects?.length == 0 ? (
              <p>No Subject found in this Test</p>
            ) : (
              ""
            )}
          </div>
          ...
          {subjects &&
            filterSubjects(subjects).map((i, d) => {
              return (
                <div className="w-full rounded-md border-1 border-gray-200 flex flex-row justify-between p-1 px-2 shadow-md items-center my-2">
                  <div className="flex flex-row items-center justify-start">
                    <p>{i?.title}</p>
                    <p className="bg-primary text-white text-xs px-4 py-1 rounded-full mx-2">
                      {i?.label ?? "No Label"}
          ...
          <Popover>
            <PopoverTrigger>
              <div className="w-full rounded-md border-1 border-gray-200 border-dashed flex flex-row justify-between p-2 px-2 bg-gray-50 items-center my-2">
                {" "}
                Add a Subject
              </div>
            </PopoverTrigger>
            ...
            <PopoverContent className="sf p-2 text-left flex flex-col justify-start items-start align-top">
              <Input
                ...
                size="sm"
                onChange={(e) => {
                  setSubjectData((res) => ({ ...res, title: e.target.value }));
                }}
              ></Input>
              <Input
                label="Subject Label"
                ...
                size="sm"
                onChange={(e) => {
                  setSubjectData((res) => ({ ...res, label: e.target.value }));
                }}
              ></Input>
              <Button
                color="primary"
        ...
        <div>
          ...
          <div className="w-full p-3 rounded-md border-1 border-gray-100 bg-gray-100 mb-6 shadow-md">
                  ...
                  <div className="flex flex-row w-full justify-between bg-white border-1 rounded-md p-2 items-center align-middle">
                    ...
                    <div className="flex flex-row">
                      ...
                      <Button
                        className="ml-2"
                        onPress={() => {
                          removeFromModule(i?.id, i?.parent_sub);
                        }}
                        color="danger"
                        size="sm"
                      >
            ...
            {selectedModules && selectedModules?.length == 0 ? (
              <p className="text-center">No Module Selected</p>
            ) : (
              ""
            )}
          </div>
                ...
                <div className="w-full rounded-md border-1 border-gray-200 flex flex-row justify-between p-1 px-2 shadow-md items-center my-2">
                  ...
                  <div>
                    <Button
                      ...
                      className="ml-1 text-white"
                      onClick={() => {
                        addModuletoTest(activeSelectedSub, i?.id);
                      }}
                    >
                      Add Module to Test
                    </Button>
                    ...
                    {role == "admin" ? (
                      <>
                        <Popover
                          onOpenChange={(e) => {
                            e == true ? setEditModule(i) : "";
                          }}
                        >
                          <PopoverTrigger>
                            <Button size="sm" color="success" className="ml-1">
                              Edit
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent>
          ...
          <Popover>
            <PopoverTrigger>
              <div className="w-full rounded-md border-1 border-gray-200 border-dashed flex flex-row justify-between p-2 px-2 bg-gray-50 items-center my-2">
                {" "}
                Add a Module to {subjects[activeSubject]?.title} of{" "}
                {courses[getSubjectIndex(activeCourse)]?.title}
              </div>
            </PopoverTrigger>
            ...
            <PopoverContent className="sf">
              <Input
                ...
                size="sm"
                onChange={(e) => {
                  setModuleData((res) => ({ ...res, title: e.target.value }));
                }}
              ></Input>
              <Input
                className="my-1"
      ...
      activeModule != undefined ? (
        <div className="flex flex-col justify-start items-start w-full overflow-y-auto pr-2">
          {questions != undefined &&
            questions.map((i, d) => {
              return (
                <div className="p-3 w-full shadow-md flex flex-row justify-between items-center align-middle rounded-md my-2 border-1 border-gray-100">
                  <div className="flex flex-row items-center justify-start">
                    <Input
                      type="number"
                      label="Serial Number"
          ...
          {role == "admin" ? (
            <div
              className="w-full cursor-pointer border-dashed border-1 rounded-md border-gray-200 p-2"
              onClick={() => {
                setQuestionModal(true);
              }}
            >
                    ...
                    <Popover>
                      <PopoverTrigger>
                        <Button
                          ...
                          className="w-auto mr-auto text-white"
                          color="primary"
                        >
                          <svg
                            width="24"
                            ...
                            return (
                              <div className="flex flex-col my-5 border-1 border-gray-300 p-5 rounded-xl">
                                ...
                                <ImageUploader
                                  data={{
                                    image:
                                      editQuestionData?.options[d]?.popupimage,
                                  }}
                                  size="small"
                                  onUploadComplete={(e) => {
                                ...
                                <Popover>
                                  ...
                                  <PopoverContent>
                                    {editQuestionData?.options &&
                                      editQuestionData?.options
                                        ?.filter((item) => item.popupimage)
                                        .map((v, z) => {
                                          return (
                                            <div className=" aspect-square w-[120px] rounded-lg">
                                              <img src={v.popupimage} />
                                              <Button
                                                size="sm"
                                                color="primary"
                                                onPress={() => {
    ...
    ></CustomEditor> */}
                      ...
                      <>
                        ...
                        <QuillWarapper
                          value={
                            editQuestionData?.options?.wintext ||
 ...
 */}
    ...
    <ImageUploader size="small" onUploadComplete={(e)=>{setAddNewQuestion(res=>({...res,answerimage:e}))}}></ImageUploader> */}
                  ...
                  <>
                    ...
                    {addNewQuestion?.type == false ? (
                      <>
                        <h2>Options/Correct Answer</h2>
                        {Array(addNewQuestion.options?.length ?? 4)
                          .fill()
                          .map((i, d) => {
                            return (
                              <div className="flex flex-col my-5 border-1 border-gray-300 p-5 rounded-xl">
                                ...
                                <ImageUploader
                                  data={{
                                    image:
                                      addNewQuestion?.options[d]?.popupimage,
                                  }}
                                  size="small"
                                  onUploadComplete={(e) => {
  ...
  options: { ...res.options, losetext: e }
    ...
    <ImageUploader size="small" onUploadComplete={(e)=>{setAddNewQuestion(res=>({...res,answerimage:e}))}}></ImageUploader> */}
                    ...
                    <Button
                      onClick={() => {
                        addQuestion(modules[activeModule]?.id);
                      }}
                      className="text-left w-auto mr-auto text-white"
                      color="primary"
                    >
                      <svg
                        width="24"
