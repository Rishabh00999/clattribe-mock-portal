...
const Selector = ({ type, onSelect, role, title }) => {
  ...
  async function addGroup(a) {
    ...
    if (!a?.image) a.image = "https://placehold.co/600x400";
    const { data, error } = await supabase
      .from("test_groups")
      .insert({ ...a, type: type })
      .select();
    ...
    <div className="w-full flex flex-col items-start justify-start">
      ...
      <div className="w-full flex flex-row flex-wrap items-stretch justify-start">
        {(groups == undefined || groups?.length == 0) && !loading ? (
          <>
            <p className="w-full p-3 rounded-xl px-4 border-dashed bg-gray-50">
        ...
        {groups &&
          groups.map((i, d) => {
            return (
              <div
                /* onClick={()=>{onSelect(i.id)}} */ className="flex mr-4 relative shadow-md rounded-md overflow-hidden flex-col !flex-grow-0 flex-[100%] sm:flex-[50%] lg:flex-[300px]"
              >
                <img
                ...
                {role == "admin" ? (
                  <Dropdown size="sm" placement="bottom-start">
                    <DropdownTrigger>
                      <MoreVertical
                        size={16}
                        className="bg-white w-12 p-4 h-12 absolute right-4 top-4 rounded-full "
                      ></MoreVertical>
                    </DropdownTrigger>
                    <DropdownMenu>
                      <DropdownItem
                        startContent={<Trash2></Trash2>}
                        color="danger"
                        onPress={() => {
                          deleteGroupbyId(i.id);
                        }}
                      >
                        Delete
                      </DropdownItem>
                ...
                <div className="p-4 flex flex-col items-start justify-start">
                  <h2 className="text-left text-primary text-lg font-bold">
                    {i.title}
                  </h2>
                  ...
                  <div className="flex flex-row items-center justify-start w-full">
                    ...
                    >
                      Access Now <ChevronRight></ChevronRight>
                    </Button>
                    <Spacer x={2}> </Spacer>
                    {role == "admin" ? (
                      <Switch
